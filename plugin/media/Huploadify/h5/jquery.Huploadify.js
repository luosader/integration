(function($){
	$.fn.Huploadify = function(opts){
		var itemTemplate = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><input type="hidden" name="file_name[]" value="${fileName}"><span class="uploadbtn">上传</span><span class="delfilebtn">删除</span></div>';// 每上传一次文件用一次
		var defaults = {
			fileTypeExts:'',//允许上传的文件类型，格式'*.jpg;*.doc'
			uploader:'',//文件提交的地址
			auto:false,//是否开启自动上传
			method:'post',//发送请求的方式，get或post
			multi:true,//是否允许选择多个文件 是指在一次选择中是否多文件
			...
			itemTemplate:itemTemplate,//上传队列显示的模板 文件列表
		...
		this.each(function(){
			...
			inputStr += '"/>';
			inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button" style="color:#1e3564;width:86px;">';
			...
			var fileObj = {
				...
				onSelect: function(files){
					for(var i=0,len=files.length;i<len;i++){
						var file = files[i];
						//处理模板中使用的变量
						// alert('文件选择后:index='+file.index+'&name='+file.name+'&size='+file.size+'&instanceID='+_this.attr('id'))
						var $html = $(
								option.itemTemplate.replace(/\${fileID}/g, 'fileupload_'+instanceNumber+'_'+file.index).replace(/\${fileName}/g, file.name).replace(/\${fileSize}/g, formatFileSize(file.size)).replace(/\${instanceID}/g, _this.attr('id'))
							);
						//如果是自动上传，去掉上传按钮
						if(option.auto){
				...
				funDeleteFile: function(index) {
					for (var i = 0,len=this.fileFilter.length; i<len; i++) {
						...
						if (file.index == index) {
							...
							// 拓展 删除掉某个文件后的回调函数，可传入参数 file,file在这里是一个对象
				...
				funUploadFile: function(file) {
					...
					if (xhr.upload) {
						...
						// 文件上传成功或是失败 会在 addEventListener 之后执行
						xhr.onreadystatechange = function(e) {
							if (xhr.readyState == 4) {
								if (xhr.status == 200) {
									// alert("服务器响应数据：" + this.responseText);// responseText 为 php端回调过来的数据
									//校正进度条和上传比例的误差
